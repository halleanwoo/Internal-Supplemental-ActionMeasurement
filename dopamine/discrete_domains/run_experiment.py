...
@gin.configurable
def create_agent(sess, environment, base_dir,agent_name=None, summary_writer=None,
                 debug_mode=False):
  """Creates an agent.
  ...
  if not debug_mode:
    summary_writer = None
  if agent_name == 'dqn':
    return dqn_agent.DQNAgent(sess, num_actions=environment.action_space.n,base_dir=base_dir,
...
class Runner(object):
  ...
  def __init__(self,
    ...
    self._agent = create_agent_fn(self._sess, self._environment,base_dir=self._base_dir,
    ...
    self.levin_max_rew_train = 0
    self.levin_max_rew_evel = 0
  ...
  def _run_one_episode(self):
    ...
    if total_reward > self.levin_max_rew_train:
      self.levin_max_rew_train = total_reward
      print("max reward : %f" % self.levin_max_rew_train)
